...
Up to task 030, the orchestrator can discover the backlog, simulate execution across real task files, and evaluate review / approval logic. However, `execute\_task()` still returns a mocked success payload. The next milestone is to let the orchestrator actually execute one backlog task.
...
\- `src/builder/orchestrator/project\_config.py`

\- `src/builder/orchestrator/project\_adapter.py`

\- `src/builder/orchestrator/cli.py`

\- `tests/test\_orchestrator\_real\_execution.py`
...
3\. `OrchestratorRunner.execute\_task()` must invoke the configured task runner.
