...
The supervised three-step canary runner and durable chain ledger were implemented in:
- `agents/lib/three_step_canary.py`

A companion test verifies acceptance of exactly three adjacent tasks (under strict adjacency), conservative rejection of malformed shapes, durable ledger persistence, and explicit stopping for blocked or incompatible chains:
- `tests/test_three_step_canary.py`
