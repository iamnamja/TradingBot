...
4. schema alias normalization through one shared normalization layer
5. canonical stop-status and decision vocabulary across batch/controller/merge surfaces
6. next-task choice driven by explicit backlog policy and dependency truth
7. stronger self-heal with bounded repair planning and rollback where relevant
8. project-aware validation and authority evaluation
9. conservative stop behavior when portfolio scheduling or merge authority is unsatisfied
...
Task 126 adds one canonical stop-status and decision vocabulary so manual-patch, blocked, stop, and merge-posture failure states normalize through shared contract helpers instead of drifting across modules.
