...
def test_ranked_repair_action_prefers_rollback_when_current_validation_regressed() -> None:
    be = _batch_executor_module()
    current = {
        "lint_ok": True,
        "test_ok": False,
    ...
    plan = be.choose_ranked_repair_action(
        repair_candidates=[
            {
                "repair_strategy": "behavioral_test_repair",
                "remediation_lane": "builder",
                "target_files": ["agents/lib/example.py"],
    ...
    assert plan["selected_action"] == "rollback_to_last_green"
    assert plan["rollback_truth"]["should_rollback_to_last_green"] is True
    assert plan["ranked_candidates"][0]["repair_strategy"] == "behavioral_test_repair"
