...
def test_partition_required_paths_excludes_meta_harness_files_from_normal_bundle() -> None:
    # Protected meta harness files must be excluded from the normal bundle partition.
    normal, protected = run_task._partition_required_paths_for_normal_bundle(
        ["agents/run_task.py", "docs/TRADINGBOT_PROJECT_STATE.md"]
    )
    assert "docs/TRADINGBOT_PROJECT_STATE.md" in normal
    assert "agents/run_task.py" not in normal
