...
def test_extractors_find_values_current_snapshot():
    ...
    assert extract_completed_through_task(readme_text) == 185
    assert extract_completed_through_task(state_text) == 185
...
def test_detects_completed_through_drift(tmp_path: Path):
    ...
    readme_text = Path("README.md").read_text(encoding="utf-8")
    # Replace the specific "complete through Task 185" phrasing; fall back to "post-Task 185" if needed
    if "complete through Task 185" in readme_text:
        mutated = readme_text.replace("complete through Task 185", "complete through Task 186")
    else:
        mutated = readme_text.replace("post-Task 185", "post-Task 186")
